...
export class PrintProformaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.proformaService.getProformaById(this.id).subscribe((response) => {
      ...
      this.generatePdf('print', this.proforma);
      this.listMonture = this.proforma.ventes.filter(value => value.libelle =="monture");
      this.listLentilleD = this.proforma.ventes.filter(value => value.libelle =="lentilleD");
      this.listLentilleG = this.proforma.ventes.filter(value => value.libelle =="lentilleG");
      this.calculValeur();
      this.proforma.ventes.forEach(v => {
        if(v.libelle == "monture"){
          this.isMonture = true;
        }
        else
        if(v.libelle == "lentilleG"){
          this.isLentilleG = true;
        }
        else if(v.libelle == "lentilleD"){
          this.isLentilleD = true;
        }
      })
  ...
  calculValeur(){
    ...
    this.taxe = 0;
    this.net = 0;
    this.proforma.ventes.forEach(value => {
            ...
            [
              {
                ...
                table: {
                  ...
                  body: [
                    ...
                    [
                      {
                        text: proforma.prescription?proforma.prescription.prescripteur.nom:'',
                        bold: true,
                      },
                    ],
                    [
                    ...
                    [
                      proforma.prescription?proforma.prescription.datePrescription:''
                    ],
                  ]
                }
              },
